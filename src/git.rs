...
impl GitManager {
  ...
  pub fn get_current_branch(&self) -> Result<String, Box<dyn std::error::Error>> {
    Ok(self.command("branch", &["--show-current"])?.trim().to_string())
  }
  ...
  fn update_branch_ref(&self, branch: &str, target: &str) -> Result<(), Box<dyn std::error::Error>> {
