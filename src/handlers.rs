...
use colored::*;
use dialoguer::{theme::ColorfulTheme, Select, MultiSelect, Confirm};
use std::process;
...
pub fn handle_delete_command(git: &GitManager) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let selected_branches: Vec<String> = selections.iter()
        .map(|&i| branches[i].name.clone())
        .collect();

    let message = if selected_branches.len() == 1 {
        format!("Are you sure you want to delete this branch?\n  {}", selected_branches[0])
    } else {
        format!("Are you sure you want to delete {} branches?\n  {}",
        ...
        .interact()?
    {
        ...
        println!("{}", format!("Deleted {} branches", selected_branches.len()).green());
    }
...
pub fn handle_sync_command(git: &GitManager) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for status in branch_statuses {
        match status {
            BranchStatus::Updated(branch) => {
                println!("{} Updated branch {} (fast-forward)", "✓".green(), branch.green());
            }
            BranchStatus::Merged(branch) => {
                println!("{} Branch {} was merged to default branch", "!".yellow(), branch);
            }
            BranchStatus::RemoteGone(branch) => {
                println!("{} Branch {} was deleted on remote but not merged", "!".red(), branch);
            }
    ...
    if !merged_branches.is_empty() {
        println!("\nFound {} merged branches that can be deleted:", merged_branches.len());
        ...
        if Confirm::with_theme(&ColorfulTheme::default())
            ...
            .interact()?
        {
            let branch_names: Vec<String> = merged_branches.into_iter()
                .map(|b| b.name)
                .collect();

            git.delete_branches(&branch_names)?;
            println!("{}", format!("Deleted {} merged branches", branch_names.len()).green());
        }
    } else {
